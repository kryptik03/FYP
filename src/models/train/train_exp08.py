...
train_exp08.py
==============
Dedicated training orchestrator for Exp08:
  Vision Transformer (ViT) backbone + SupCon Pre-Training & Semi-Supervised
  Spherical DEC on 2D Bispectrum representations.
...
3.  *** INTERACTIVE EPOCH SELECTION ***
    ...
    and Phase 2 epochs.  The YAML config values serve as defaults — press
    Enter to accept them without typing anything.
    The tuning epochs and dataset subset size remain hardcoded (not interactive).

Usage:
    python src/models/train/train_exp08.py --config src/models/configs/tune_exp08_dec.yaml

    # Skip the interactive prompt (use YAML values directly):
    python src/models/train/train_exp08.py --config ... --no_prompt

    # Quick sanity check (1 shard, 1+1 epochs, batch=4):
    python src/models/train/train_exp08.py --config ... --smoke_test

After training completes:
  - Weights:         models/weights/model_<NodeID>.pt
  - Config snapshot: models/configuration_snapshots/config_<NodeID>.yaml
  - Run folder:      data/performance_evaluation/training/<ts>_<origin>-<root>-<node>/
      ...
      * fig_loss_phase1.png       — Phase 1 SupCon train/val loss curves
      * fig_loss_phase2.png       — Phase 2 KL-div / Pairwise / Total train/val curves
      * fig_epoch_times.png       — Per-epoch wall-clock times
      * fig_grad_norms.png        — Per-epoch gradient norms by phase
  - Lineage:         src/utils/lineage.db
  - Latest node:     src/utils/latest_node.txt
"""
...
def write_latest_node(node_id: str):
    """Write the node ID to src/utils/latest_node.txt for Colab commit messages."""
...
def get_grad_norm(model: torch.nn.Module) -> float:
    """Compute the global L2 gradient norm across all trainable parameters."""
    total_norm = 0.0
...
def prompt_epochs(
    ...
    *** INTERACTIVE EPOCH SELECTION ***
    ...
    Asks the user to input Phase 1 and Phase 2 epoch counts at runtime.
    The YAML-configured values are shown as defaults — pressing Enter uses them.

    Args:
        default_p1 : Default Phase 1 epochs (from YAML config).
        default_p2 : Default Phase 2 epochs (from YAML config).
        no_prompt  : If True, skip the prompt and use defaults directly.

    Returns:
        (phase1_epochs, phase2_epochs) as integers.
    """
    if no_prompt:
    ...
    while True:
        try:
            raw = input(f"  Phase 1 (SupCon) epochs [{default_p1}]: ").strip()
            phase1_epochs = int(raw) if raw else default_p1
            if phase1_epochs < 1:
    ...
    while True:
        try:
            raw = input(f"  Phase 2 (DEC)    epochs [{default_p2}]: ").strip()
            phase2_epochs = int(raw) if raw else default_p2
            if phase2_epochs < 1:
                print("  Please enter a positive integer.")
                continue
            break
...
# ---------------------------------------------------------------------------
# Plotting (identical dark-theme style to train_exp07.py)
# ---------------------------------------------------------------------------
...
def plot_phase1_loss(history: dict, out_dir: str, phase1_epochs: int) -> str:
    """Phase 1 SupCon train/val loss curves (dark theme)."""
    epochs = list(range(1, phase1_epochs + 1))
    fig, ax = plt.subplots(figsize=(9, 4))
    _apply_dark_style(fig, ax)

    ax.plot(epochs, history["p1_train_supcon"], color=COLORS[0], lw=2, label="Train SupCon")
    ax.plot(epochs, history["p1_val_supcon"],   color=COLORS[1], lw=2, linestyle="--", label="Val SupCon")

    best_ep = int(np.argmin(history["p1_val_supcon"])) + 1
    ax.axvline(best_ep, color="#FFBE0B", lw=1, linestyle=":", alpha=0.8, label=f"Best Val (ep {best_ep})")

    ax.set_xlabel("Epoch")
    ax.set_ylabel("SupCon Loss")
    ax.set_title(
        "Phase 1 — Supervised Contrastive Pre-Training Loss (ViT + Bispectra)",
...
def plot_phase2_loss(history: dict, out_dir: str, phase2_epochs: int) -> str:
    """Phase 2 KL / Pairwise / Total train+val loss curves (dark theme)."""
    ...
    fig.suptitle(
        "Phase 2 — Semi-Supervised DEC Training Curves (Bispectrum ViT)",
...
def plot_epoch_times(epoch_times: list, phase1_epochs: int, out_dir: str) -> str:
    """Bar chart of wall-clock time per epoch, colour-coded by phase."""
    n = len(epoch_times)
    ...
    ax.legend(
        handles=[
            Patch(facecolor=COLORS[0], label="Phase 1 (SupCon)"),
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Exp08: SupCon & Semi-Supervised DEC on 2D Bispectrum + ViT."
    )
    parser.add_argument(
        "--config", required=False,
        default="src/models/configs/tune_exp08_dec.yaml",
        help="Path to the exp08 YAML config file.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Skip the interactive epoch prompt and use YAML config values directly.",
    )
    ...
    # --- Smoke test overrides (hardcoded for rapid validation) --- #
    if args.smoke_test:
    ...
    parent_id      = (
        tuning_node_id
        if tuning_node_id and tuning_node_id != "manual"
        else exp_cfg["parent_node_id"]
    )
    print(f"[Lineage] NodeID: {node_id}  |  Parent: {parent_id}")

    # Resolve origin + root_id from lineage DB for the run folder name
    try:
        import sqlite3
    ...
    # Create the structured performance evaluation output directory
    ...
    # -----------------------------------------------------------------------
    # *** INTERACTIVE EPOCH SELECTION ***
    # Default values come from the YAML; user can override at runtime.
    # Smoke test always skips the prompt (uses the overridden values above).
    # -----------------------------------------------------------------------
    yaml_p1 = train_cfg.get("phase1_epochs", 150)
    yaml_p2 = train_cfg.get("phase2_epochs", 150)
    ...
    # -----------------------------------------------------------------------
    # Build Datasets — Phase 1 (augmented bispectrum pairs for SupCon)
    # -----------------------------------------------------------------------
    ...
    train_loader_p1 = DataLoader(
        train_ds_p1, batch_size=bs, shuffle=True, drop_last=True, **loader_kwargs
    )
    val_loader_p1 = DataLoader(
        val_ds_p1,   batch_size=bs, shuffle=False, **loader_kwargs
    )
    ...
    # Build Model
    # -----------------------------------------------------------------------
    task = SupConDECTask_Exp08(config).to(device)
    ...
    for epoch in range(1, phase1_epochs + 1):
        ...
        print(
            ...
            f"Train SupCon: {train_losses.get('supcon', 0.0):.4f} | "
            f"Val SupCon: {val_metrics.get('supcon', 0.0):.4f} | "
            f"GradNorm: {epoch_grad_norm:.4f} | {elapsed:.1f}s"
        )

        if val_metrics.get("total", float("inf")) < best_val_loss_p1:
    ...
    with torch.no_grad():
        for batch in init_loader:
            sig = batch[0].to(device)   # (B, 1, 224, 224)
            z   = task.backbone(sig)
    ...
    train_loader_p2 = DataLoader(
        train_ds_p2, batch_size=bs, shuffle=True, drop_last=True, **loader_kwargs
    )
    val_loader_p2 = DataLoader(
        val_ds_p2, batch_size=bs, shuffle=False, **loader_kwargs
    )
    ...
    for epoch in range(1, phase2_epochs + 1):
        ...
        if val_metrics.get("total", float("inf")) < best_val_loss_p2:
    ...
    timing_report = {
        ...
        "vit_variant"           : config["model"].get("vit_variant", "vit_b_16"),
        # ── Hyperparameters ───────────────────────────────────────────────────
    ...
    tuning_note  = (
        f" | Tuned by node {train_cfg['tuning_node_id']} @ {train_cfg.get('tuned_at', '?')}"
        if "tuning_node_id" in train_cfg else ""
    )
    perf_note = (
        f" | BestValP1: {best_val_loss_p1:.4f}" if history["p1_val_supcon"] else ""
    ) + f" | BestValP2: {best_val_loss_p2:.4f} | BestEpoch: {best_epoch}"

    new_node = register_process(
        parent_id        = parent_id,
        stage            = "classification",
        method           = "supcon_dec_semi_vit_bispectrum",
        folder_path      = (
