...
train_exp07.py
==============
Dedicated training orchestrator for Exp07:
  Supervised Contrastive Learning (SupCon) & Semi-Supervised DEC on 2D STFT features.

Usage:
    python src/models/train/train_exp07.py --config src/models/configs/tune_exp07_dec.yaml
...
def plot_phase1_loss(history: dict, out_dir: str, phase1_epochs: int) -> str:
    """Phase 1 SupCon train/val loss curves."""
    ...
    ax.set_title("Phase 1 — Supervised Contrastive Pre-Training Loss", color="white", fontsize=12, fontweight="bold", pad=10)
...
def main():
    parser = argparse.ArgumentParser(description="Train Exp07: SupCon & Semi-Supervised DEC on STFT features")
    parser.add_argument("--config", required=False,
                        default="src/models/configs/tune_exp07_dec.yaml",
                        help="Path to the exp07 YAML config.")
    ...
    # -----------------------------------------------------------------------
    # Build Datasets (Phase 1 — augmented pairs for SupCon)
    # -----------------------------------------------------------------------
    label_fraction = data_cfg.get("label_fraction", 0.10)

    train_ds_p1 = DECDataset_Exp07(
    ...
    # Build Model
    # -----------------------------------------------------------------------
    task = SupConDECTask(config).to(device)
    ...
    phase1_epochs = train_cfg.get("phase1_epochs", 150) if args.smoke_test else 150
    phase2_epochs = train_cfg.get("phase2_epochs", 150) if args.smoke_test else 150
    ...
    for epoch in range(1, phase1_epochs + 1):
        ...
        history["p1_train_supcon"].append(round(train_losses["supcon"], 6))
        history["p1_val_supcon"].append(round(val_metrics["supcon"], 6))
        history["p1_grad_norms"].append(round(epoch_grad_norm, 6))
        print(f"  P1 Epoch {epoch:03d}/{phase1_epochs} | "
              f"Train SupCon: {train_losses['supcon']:.4f} | "
    ...
    train_ds_p2 = DECDataset_Exp07(
        sources=data_cfg["sources"], shard_key="train_shards",
        max_pulse_len=data_cfg["max_pulse_len"], augment=False,
        label_fraction=label_fraction,
    )
    val_ds_p2 = DECDataset_Exp07(
