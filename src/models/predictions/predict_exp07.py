...
def parse_sources(raw_list: list[str]) -> list[dict]:
    ...
    for raw in raw_list:
        ...
        sources.append({
            "type":         stype,
            "path":         path,
            "train_shards": shards,   # DECDataset_Exp07 always reads via 'train_shards'
            "val_shards":   shards,
        })
    return sources
...
def plot_soft_assignment_heatmap(results: list[dict], out_dir: str) -> str:
    ...
    This is particularly useful for exp07 since the DEC soft assignments are a
...
def main():
    ...
    history = (
        f"Semi-Supervised DEC Inference (exp07) | Checkpoint: {args.checkpoint_id} | "
