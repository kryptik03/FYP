...
def build_class_scores_from_dec(results: list[dict]) -> tuple[np.ndarray, int]:
    ...
    (e.g. exp06/07/08 with 2 classes), individual class scores will be high;
    ...
    # Hard-assignment alignment: DEC cluster -> majority GT class
    ...
    # Accumulate q_soft contributions per semantic class
    ...
    for pulses in scene_groups.values():
        for idx, feat in pulses:
            ...
            if mode == "greedy":
                ...
                seed_ch = feat["ch_idx"]
                # Collect candidates by channel (must pass both thresholds)
                by_channel: dict = defaultdict(list)  # ch -> [(jdx, dist), ...]
                for jdx, feat_j in pulses:
                    if jdx == idx or jdx in assigned_global:
                        continue
                    t_j = feat_j["start_idx"] * feat_j["time_res"]
                ...
                # For each channel other than the seed's, pick the closest candidate
...
def compute_metrics(results: list[dict], hdb_labels: np.ndarray, cluster_map: dict,
                    class_scores: np.ndarray = None,
                    inference_time_s: float = 0.0,
                    reduce_time_s:    float = 0.0,
                    hdbscan_time_s:   float = 0.0) -> dict:
    """
    Comprehensive evaluation metrics for exp06 DEC inference.

    Parameters
    ----------
    results          : list of per-pulse dicts (output of group_pd_instances).
    hdb_labels       : HDBSCAN cluster assignment per pulse.
    cluster_map      : HDBSCAN cluster id -> GT class id mapping.
    class_scores     : (N, n_classes) probability matrix from build_class_scores_from_dec.
    inference_time_s : wall-clock seconds used for feature extraction.
    """
    ...
    # ---- Per-class stats (precision / recall / F1) ----
...
def plot_confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray,
                          out_dir: str, exp_tag: str = "exp06") -> str:
...
def plot_calibration_curve(y_true: np.ndarray, y_pred: np.ndarray,
                           ...
                           out_dir: str, exp_tag: str = "exp06") -> str:
    """
    Reliability diagram: model confidence (max class score) vs observed accuracy.
    ...
    confidence = cs.max(axis=1)          # model's self-reported confidence
    correct    = (yt == yp).astype(int)  # 1 = correctly classified
...
def plot_time_vs_distance(results: list[dict], out_dir: str,
                          exp_tag: str = "exp06", max_pairs: int = 10_000) -> str:
    """
    Scatter of intra-scene pulse pairs: time difference (X) vs L2 embedding
    distance (Y).  Green = same GT instance; Red = different GT instance.
    Useful for visually calibrating time_threshold and dist_threshold.
    Pairs are uniformly sub-sampled to max_pairs for speed.
    """
    scenes: dict = defaultdict(list)
    for r in results:
        scenes[(r["shard_path"], r["scene_idx"])].append(r)
...
def main():
    ...
    print(f"[Timing] Inference: {inference_time_s:.2f}s  "
          f"({len(features) / inference_time_s:.1f} samples/s)")
    ...
    # 5. Build class probability scores from DEC q_soft
    ...
    plot_confusion_matrix(y_true, y_pred, out_dir, exp_tag="exp06")
    plot_pr_curves(y_true, class_scores, out_dir, exp_tag="exp06")
    plot_roc_curves(y_true, class_scores, out_dir, exp_tag="exp06")
    plot_calibration_curve(y_true, y_pred, class_scores, out_dir, exp_tag="exp06")
    plot_time_vs_distance(results, out_dir, exp_tag="exp06")
