...
def extract_features(task: SupConDECTask_Exp08, loader: DataLoader,
    ...
    # Cache for labels array to extract scene_idx and start_idx without reopening H5 constantly
    shard_labels_cache = {}
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for b in range(sig.shape[0]):
                ...
                # DECDataset_Exp08 index unpacks differently than Exp07
                (shard_path, pulse_idx, reported_class, actual_class, inst_id, time_res) = dataset.index[global_idx]

                # Fetch extra metadata from the shard for grouping
                ...
                records.append({
                    ...
                    "start_idx":    start_idx,
                    "pulse_idx":    pulse_idx,
                    "gt_class_id":  actual_class,     # true label
                    "gt_inst_id":   inst_id,
                    "time_res":     time_res,
                    "emb":          z[b],
                    "soft_q":       q[b],
