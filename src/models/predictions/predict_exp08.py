...
predict_exp08.py
================
Inference + Visualization script for Exp08 SupCon & Semi-Supervised DEC.
Uses a Vision Transformer (ViT) backbone on 2D Bispectrum features.

Usage:
    python src/models/predictions/predict_exp08.py \\
        --checkpoint_id <NODE_ID> \\
        --source "data/features/bispectra/20260601_123456-cw-TuKT-vraW:cwru:14,15,16" \\
        --source "data/features/bispectra/20260601_123502-sy-0s6o-UI2r:equation:17,18,19,20" \\
        [--min_cluster_size 5] \\
...
Key differences from Exp07:
  - Sources point to data/features/bispectra/ (2D Bispectrum features), NOT stft_magnitude.
  - Uses DECDataset_Exp08 and SupConDECTask_Exp08 (ViT).
  - Fetches scene_idx and start_idx from the HDF5 labels array on-the-fly for instance grouping.
...
def load_weights(checkpoint_id: str, task: SupConDECTask_Exp08):
...
def extract_features(task: SupConDECTask_Exp08, loader: DataLoader,
                     dataset: DECDataset_Exp08, device: torch.device) -> list[dict]:
    task.eval()
    records = []
    global_idx = 0

    # Cache for labels array to avoid reopening the same H5 file repeatedly
    shard_labels_cache = {}
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for b in range(sig.shape[0]):
                ...
                # DECDataset_Exp08 index tuple
                (shard_path, pulse_idx, reported_class, actual_class, inst_id, time_res) = dataset.index[global_idx]

                # Fetch scene_idx, ch_idx, start_idx from the shard labels array
                if shard_path not in shard_labels_cache:
                    with h5py.File(shard_path, "r") as f:
                        shard_labels_cache[shard_path] = f["labels"][:]

                labels_arr = shard_labels_cache[shard_path]
                scene_idx  = int(labels_arr[0, pulse_idx])
                ch_idx     = int(labels_arr[1, pulse_idx])
                start_idx  = int(labels_arr[5, pulse_idx])
...
def run_hdbscan(features: list[dict], min_cluster_size: int = 5,
    ...
    if reduce_method == "pca" and reduce_dims > 0 and embs.shape[1] > reduce_dims:
        ...
        if reduce_dims > 3:
            print(f"[Warning] t-SNE only supports up to 3 dimensions efficiently. "
                  f"Clamping reduce_dims from {reduce_dims} to 3.")
            reduce_dims = 3
...
def build_class_scores_from_dec(results: list[dict]) -> tuple[np.ndarray, int]:
    ...
    (e.g. exp08 with 2 classes), individual class scores will be high;
    ...
    if class_scores is not None and len(unique_classes) >= 2 and n_cls >= 2:
        try:
            ...
            if len(labels_present) == 2:
                auc_roc_macro = float(roc_auc_score(y_true == labels_present[1], cs_sub[:, 1]))
            else:
...
def plot_embeddings(results: list[dict], hdb_labels: np.ndarray,
    ...
    plt.suptitle("Exp08 SupCon & Semi-Supervised DEC (ViT) — Embedding Space",
...
def plot_confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray,
                          out_dir: str, exp_tag: str = "exp08") -> str:
...
def plot_time_vs_distance(results: list[dict], out_dir: str,
                          exp_tag: str = "exp08", max_pairs: int = 10_000) -> str:
    scenes: dict = defaultdict(list)
    for r in results:
        scenes[(r["shard_path"], r["scene_idx"])].append(r)
...
def main():
    parser = argparse.ArgumentParser(
        description="Exp08 SupCon & Semi-Supervised DEC (ViT): Inference, HDBSCAN Clustering, Visualization"
    )
    ...
    parser.add_argument("--source", action="append", dest="sources", default=[],
                        ...
                        help="Bispectrum feature source. Repeatable. Format: path:type:shard1,shard2,...")
    ...
    task = SupConDECTask_Exp08(config)
    ...
    # 3. Extract embeddings + soft assignments (timed)
    ...
    print(f"[Alignment] Cluster->Class map: "
    ...
    results_base = os.path.abspath(out_cfg.get("results_dir", "data/classification_output/exp08_supcon_dec_vit"))
    ...
    with h5py.File(h5_path, "w") as f:
        ...
        keys_to_save = ["shard_path", "scene_idx", "ch_idx", "start_idx",
                        ...
                        "gt_inst_id", "pred_inst_id", "time_res", "pulse_idx"]
    ...
    plot_confusion_matrix(y_true, y_pred, out_dir, exp_tag="exp08")
    plot_pr_curves(y_true, class_scores, out_dir, exp_tag="exp08")
    plot_roc_curves(y_true, class_scores, out_dir, exp_tag="exp08")
    plot_calibration_curve(y_true, y_pred, class_scores, out_dir, exp_tag="exp08")
    plot_time_vs_distance(results, out_dir, exp_tag="exp08")

    # 13. Lineage registration
    history = (
        f"Semi-Supervised DEC Inference (Exp08 ViT) | Checkpoint: {args.checkpoint_id} | "
    ...
    register_process(
        ...
        stage            = "prediction",
        method           = "dec_semi_hdbscan_vit_bispectrum",
        folder_path      = out_dir,
