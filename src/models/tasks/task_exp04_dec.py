...
task_exp04_dec.py
=================
Two-Phase Deep Embedded Clustering (DEC) Task.
...
PHASE 2  — DEC Unsupervised Cluster Refinement
-----------------------------------------------
...
Post-Training / Inference:
  Labels (class_id) from the dataset are used ONLY post-hoc to map
  discovered cluster IDs → real class names. HDBSCAN (in predict_exp04.py)
  is used to find the final number of clusters at inference time.
"""
...
from src.models.models.backbones.backbone_exp04_dec import DECCNN1D
...
class DECTask(nn.Module):
    ...
    Exposes:
        ...
        training_step_phase2(batch) -> (q, loss_dict)     for DEC
    ...
    def soft_assign(self, z: torch.Tensor) -> torch.Tensor:
        ...
        diff = z.unsqueeze(1) - self.cluster_layer.unsqueeze(0)   # (B, K, D)
    ...
    def init_cluster_centroids(self, all_embeddings: np.ndarray):
        ...
        print(f"[DEC] Cluster centroids initialized.")
