...
def sensors_data(data_predict: np.ndarray=np.array([])) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float, np.ndarray]:
    ...
    # Time unit is in ns
    if data_predict.size == 0:
        # use example values if data_predict is not provided
...
def sorting_data_predict(data_predict: np.ndarray, data_set: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    TDOA_N = data_predict[data_set-1, 3:]  # Convert to seconds
...
def pso_algorithm(data_predict: np.ndarray=np.array([])):
    """Main PSO algorithm implementation."""
    # Get sensor data
    receiver1, receiver2, receiver3, receiver4, speed_of_EM, data_predict = sensors_data(data_predict)
    ...
    for data_set in range(1, num_of_loop + 1):
        ...
        for iter_pso in range(max_iterations_pso):
            ...
            particles_position = np.clip(particles_position, [0.5, 0.5, 0.1], [11, 11, 11])
