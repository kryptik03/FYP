...
def simulate_SEDO(t, V0, tau, fc, theta=0):
    """
    ...
    """
    signal = V0 * np.exp(-t / tau) * np.sin(2 * np.pi * fc * t + theta)
    return signal

def simulate_DED(t, V0, alpha, beta):
    """
    ...
    """
    signal = V0 * (np.exp(-alpha * t) - np.exp(-beta * t))
    return signal

def simulate_DEDO(t, V0, alpha, beta, fc, theta=0):
    """
    ...
    theta : Phase shift (radians)
    """
    signal = V0 * (np.exp(-alpha * t) - np.exp(-beta * t)) * np.sin(2 * np.pi * fc * t + theta)
    return signal

def simulate_SMG(t, A, t0, tau, fc):
    """
    ...
    """
    exponent = -((t - t0)**2) / (2 * (tau**2))
    signal = A * np.exp(exponent) * np.sin(2 * np.pi * fc * (t-t0))
    return signal
...
if __name__ == "__main__":
    ...
    # --- Generate Signals ---
    y_sedo = simulate_SEDO(t, V0_sedo, tau_sedo, fc_sedo)
    y_ded = simulate_DED(t, V0_ded, alpha_ded, beta_ded)
